...
const ModalWrapper = ({ onClose, children }) => (
    <motion.div
        ...
        className="fixed inset-0 bg-black/75 backdrop-blur-md flex justify-center items-center z-50 p-4"
        onClick={onClose}
    >
        ...
        <div className="relative w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="animated-border absolute -inset-[2px] rounded-2xl pointer-events-none" />

            <motion.div
                initial={{ y: 60, opacity: 0, scale: 0.96, filter: "blur(8px)" }}
                animate={{ y: 0, opacity: 1, scale: 1, filter: "blur(0px)" }}
                exit={{ y: 50, opacity: 0, scale: 0.95, filter: "blur(4px)" }}
                transition={{ type: "spring", damping: 26, stiffness: 320 }}
                className="relative glass-card w-full overflow-hidden !p-0 rounded-2xl shadow-2xl"
            >
                ...
                {authStep === 'selection' ? (
                    ...
                    <div className="grid gap-4 py-2">
                        ...
                        <button
                            ...
                            className="group relative flex items-center p-4 rounded-2xl border border-slate-700/50 bg-slate-800/20 hover:bg-slate-800/60 transition-all duration-300 hover:border-violet-500/50 hover:shadow-[0_0_30px_rgba(139,92,246,0.1)]"
                        >
                            <div className="absolute inset-0 bg-gradient-to-r from-violet-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl" />

                            <div className="h-14 w-14 rounded-full bg-slate-900 border border-slate-700/80 group-hover:border-violet-500/30 flex items-center justify-center mr-5 relative z-10 shadow-lg group-hover:shadow-violet-500/20 transition-all">
                                <Microscope className="text-slate-400 group-hover:text-violet-400 transition-colors" size={24} />
                            </div>

                            <div className="text-left flex-1 relative z-10">
                                <h3 className="text-lg font-bold text-slate-200 group-hover:text-white transition-colors">Researcher Portal</h3>
                                <p className="text-xs font-medium text-slate-500 group-hover:text-violet-500/80 transition-colors">Data & Model Access</p>
                            </div>

                            <div className="h-10 w-10 rounded-full bg-slate-900/50 flex items-center justify-center text-slate-500 group-hover:text-violet-500 group-hover:bg-violet-500/10 transition-all border border-transparent group-hover:border-violet-500/20">
                                <ArrowRight size={18} className="group-hover:translate-x-0.5 transition-transform" />
                            </div>
                        </button>
                    </div>
                            ...
                            <div className="flex items-center gap-3 p-3 bg-slate-800/30 rounded-xl border border-white/5">
                                <div className="relative flex items-center justify-center shrink-0">
                                    <input
                                        type="checkbox"
                                        id="termsConsent"
                                        className="peer w-5 h-5 appearance-none rounded border border-slate-600 bg-slate-800 checked:bg-amber-500 checked:border-amber-500 transition-colors cursor-pointer"
                                    />
                                    <svg className="absolute w-3.5 h-3.5 text-black opacity-0 peer-checked:opacity-100 pointer-events-none transition-opacity" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                                </div>
                                <label htmlFor="termsConsent" className="text-xs text-slate-400 leading-tight cursor-pointer select-none">
                                    I agree to the <a href="/terms" target="_blank" className="text-amber-500 hover:text-amber-400 font-medium hover:underline">Terms of Service</a> and <a href="/privacy" target="_blank" className="text-amber-500 hover:text-amber-400 font-medium hover:underline">Privacy Policy</a>.
                                </label>
                            </div>
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    if (consent && consent.checked) {
                                        ...
                                        // Shake effect via error state if I had animation for it, but error message is enough
                                    }
                                }}
