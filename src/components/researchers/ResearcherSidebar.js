...
import { LayoutDashboard, Database, Layers, Brain, X, ArrowLeft, Settings, Shield, Globe } from '../Icons'; // Added Globe/ArrowLeft/etc.
import CureBirdLogo from '../../curebird_logo.png'; // Adjusted path
import useHaptics from '../../hooks/useHaptics'; // Adjusted path

const ResearcherSidebar = ({ isOpen, onClose, user, onBackToApp }) => {
    ...
    const menuGroups = [
        {
            ...
            items: [
                ...
                { name: 'Data Explorer', path: '/research/data', icon: Database },
                { name: 'Model Repo', path: '/research/models', icon: Layers },
                { name: 'Playground', path: '/research/playground', icon: Brain },
            ]
        },
        {
            title: "System",
            items: [
                { name: 'Back to App', path: null, icon: ArrowLeft, isAction: true, action: onBackToApp },
            ]
        }
    ];
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center justify-center gap-3 shadow-lg backdrop-blur-md transition-all duration-300 hover:bg-white/10 group/card">
                        ...
                        <div className="relative group shrink-0">
                            ...
                            <div className="relative w-12 h-12 rounded-full bg-black/40 p-2 flex items-center justify-center transition-transform duration-500 group-hover/card:scale-110">
                                {/* The Logo Itself - Using same logo as main app */}
                                <img
                                    src={CureBirdLogo}
                                    alt="CureBird Logo"
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-amber-500/80 font-bold uppercase tracking-wider">Research Portal</p>
                        </div>
                    </div>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 bg-black/40 border-t border-white/5 backdrop-blur-md">
                        <div className="flex items-center gap-3 group cursor-pointer hover:bg-white/5 p-2 rounded-xl transition-colors">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="text-xs text-amber-500/80 font-medium">Research Access</p>
                            </div>
                        </div>
                    </div>
                )}
