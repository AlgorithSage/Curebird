...
import { FileText, Activity } from '../Icons';
...
const DoctorSummaryView = ({ user, disease, metrics = [], insights, medications = [] }) => {
    ...
    // NOTE: kept logic but removed unused variable to satisfy ESLint
    ...
    const stats = useMemo(() => {
        ...
        const values = metrics.map(m => Number(m.value)).filter(v => !Number.isNaN(v));
        ...
        const lastDate = metrics[0]?.timestamp?.seconds
            ? new Date(metrics[0].timestamp.seconds * 1000).toLocaleDateString()
            : 'N/A';
            ...
            <div className="border-b-2 border-slate-900 pb-4 mb-6 flex justify-between items-start">
                <div>
                    ...
                    <div className="text-sm text-slate-500 mt-1">
                        Generated by CureBird Patient Intelligence Platform
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                ...
                <div>
                    <h2 className="text-sm font-bold uppercase text-slate-500 mb-3 border-b border-slate-200 pb-1 flex items-center gap-2">
                        <TrendingUpIcon /> Recent Vitals (n={stats?.count || 0})
                    </h2>
...
const BotIcon = () => (
    ...
    >
        ...
        <path d="M5 22h14" />
        <path d="M5 10V4a2 2 0 0 0-2-2" />
        <path d="M19 10V4a2 2 0 0 1 2-2" />
    </svg>
);
