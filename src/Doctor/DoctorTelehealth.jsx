...
import { 
    ...
    ChevronLeft, ChevronRight, AlertCircle, CheckCircle, XCircle, MoreVertical,
...
const DoctorTelehealth = ({ onNavigate, patients = [] }) => {
    ...
    const slots = React.useMemo(() => {
        ...
        return allTimes.map(time => {
            ...
            if (bookedAppt) return { time, status: 'booked', patient: bookedAppt.patientName, id: bookedAppt.id };
            const isExtra = extraSlots.find(s => s.time === time);
            if (isExtra) return { time, status: 'available' };
            if (time === '12:00 PM' || time === '12:30 PM') return { time, status: 'blocked' };
    ...
    const handleAction = (action, appt) => {
        if (action === 'enter_room') {
            ...
            setApprovedAppts(prev => [...prev, { ...appt, type: 'video', date: 'Today' }]);
        }
    ...
    const tabs = [
        ...
        { id: 'requests', label: 'Requests', icon: AlertCircle, badge: requests.length },
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                {/* Tab Navigation */}
                <div className="flex p-1 bg-stone-900/50 backdrop-blur-md border border-white/5 rounded-xl">
                    {tabs.map(tab => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`relative px-6 py-2.5 rounded-lg text-sm font-bold uppercase tracking-wider transition-all flex items-center gap-2 ${
                                activeTab === tab.id 
                        ...
                        >
                            ...
                            {tab.label}
                            {tab.badge > 0 && (
                                <span className={`ml-1 text-[10px] px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? 'bg-black/20 text-black' : 'bg-rose-500 text-white'}`}>
                                    {tab.badge}
                                </span>
                            )}
                        </button>
                ...
                <AnimatePresence mode="wait">
                    ...
                    {activeTab === 'schedule' && (
                        ...
                        >
                             ...
                             <div className="glass-card p-8 rounded-[2.5rem] bg-[#080705] border border-white/5 relative overflow-hidden">
